...
def get_google_search_results_selenium(keyword, latitude=None, longitude=None, driver=None):
    ...
    try:
        ...
        return {'query': keyword, 'autofill_suggestions': [], 'paa_questions': [], 'related_searches': [], 'error': str(e)}
    except TimeoutException as e:
        print(f"Timeout occurred during scraping: {e}")
        return {'query': keyword, 'autofill_suggestions': [], 'paa_questions': [], 'related_searches': [], 'error': str(e)}
...
def recursive_search(keyword, depth=1, max_depth=3, driver=None, latitude=None, longitude=None):
    ...
    if depth < max_depth:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_term = {executor.submit(fetch_recursive_search_data, term, depth, max_depth, driver, latitude, longitude): term for term in terms}
